...
def filter_germ(somvar_dict, vcf_file, threads=1, return_all_germline_IDs=False):
    """
    ...
    the set of germline sites, a list of filtered germline sites per sample.
    ...
    for key, item in somvar_dict.items():
        ...
        filtered_germ_sites =  filtered_germ_sites + sites
        filtered_germ_sites = list(set(filtered_germ_sites))
    if return_all_germline_IDs==True:
        return filtered_germ_sites, IDs
    else:
        return filtered_germ_sites